...
import { ArrowLeft, Mail, Lock, AlertCircle, Loader2, Apple, Eye, EyeOff } from 'lucide-react';
...
const Login = () => {
  ...
  const [showNotFoundModal, setShowNotFoundModal] = useState(searchParams.get('error') === 'account_not_found');
  ...
  const getGoogleAuthUrl = useAuthStore((s) => s.getGoogleAuthUrl);
  ...
  const handleLogin = async (e) => {
    ...
    if (user) {
      if (user.tenant_id) {
        ...
        navigate(redirectTo || PATHS.pricing, { replace: true });
      }
    }
  };
  ...
  return (
    <div className="min-h-screen flex bg-[#0f0f11] font-sans text-white">
          ...
          {!showEmailForm ? (
            <div className="w-full flex flex-col gap-3">
              ...
              <button
                ...
                disabled={isLoading}
                className="w-full bg-[#202022] hover:bg-[#2a2a2c] border border-white/5 rounded-[8px] p-2 flex items-center justify-between transition-colors shadow-sm disabled:opacity-50"
              >
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex flex-col items-start leading-tight">
                    <span className="text-[13px] font-medium text-white">{t('auth.continue_google') || 'Continue with Google'}</span>
                    <span className="text-[11px] text-white/50">{t('auth.google_sso') || 'Single Sign-On'}</span>
                  </div>
                </div>
              ...
              <div className="flex flex-col gap-1.5 mt-2">
                ...
                <div className="relative group">
                  ...
                  <input
                    ...
                    className="w-full bg-transparent border border-white/20 rounded-[8px] py-3 pl-10 pr-12 text-[13px] text-white placeholder-white/20 focus:outline-none focus:border-white transition-all font-light"
                  />
                  <button
                    type="button"
      ...
      <ConfirmModal
        ...
        onConfirm={() => navigate(PATHS.signup)}
        title="Account Not Found"
        description="No account matches this Google email. Would you like to create your network now?"
        confirmText="Sign up today"
